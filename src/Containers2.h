...
template <typename T, typename A, u64 bucketSize>
T *BucketArrayAddLockless(BucketArrayMT<T, A, bucketSize> *bucketArray)
{
	ASSERT(bucketArray->buckets.size > 0);

	u64 idx = AtomicIncrement(&bucketArray->size) - 1;

	u64 idxInsideBucket = idx % bucketSize;
	u64 bucketIdx = idx / bucketSize;
	if (bucketIdx < bucketArray->buckets.size)
		return &bucketArray->buckets[bucketIdx][idxInsideBucket];
	else
	{
		if (idxInsideBucket == 0)
		{
			// First one in new bucket. Allocate a new bucket.
			T *newBucket = A::Alloc(sizeof(T) * bucketSize);
			DynamicArrayAddMT(&bucketArray->buckets, newBucket);
			return (*newBucket)[idxInsideBucket];
		}
		else
		{
			// Someone else is trying to allocate the new bucket. Just wait a little.
			while (bucketIdx >= bucketArray->buckets.size)
				_mm_pause();
			return &bucketArray->buckets[bucketIdx][idxInsideBucket];
		}
	}
